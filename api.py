...
class UserUpdate(BaseModel):
    fund_name: Optional[str] = Field(None, min_length=3, max_length=100)
    fund_description: Optional[str] = Field(None, max_length=500)
...
class TradingTask(BaseModel):
    ...
    strategy_type: Optional[str] = Field(None, description="Trading strategy type")
    risk_level: Optional[int] = Field(None, ge=1, le=10, description="Risk level from 1-10")
...
class AutoHedgeAPI:
    def __init__(self):
        ...
        self._setup_routes()
        logger.info("AutoHedge API initialized with enhanced features")
    ...
    async def _get_current_user(self, api_key: str = Security(APIKeyHeader(name="X-API-Key"))) -> User:
    ...
    def _calculate_performance_metrics(self, trade_result: Dict) -> Dict[str, float]:
    ...
    def _setup_routes(self):
        ...
        async def get_current_user(current_user: User = Depends(self._get_current_user)):
            return current_user
        ...
        async def update_user(
            ...
            with self._log_execution_time("update_user", current_user.id):
                updated_data = current_user.dict()
                updated_data.update(user_update.dict(exclude_unset=True))
        ...
        async def create_trade(
            ...
            with self._log_execution_time("create_trade", current_user.id):
                ...
                try:
                    ...
                    trade_response.executed_at = datetime.now(timezone.utc)
                    trade_response.result = result
                    trade_response.performance_metrics = self._calculate_performance_metrics(result)
        ...
        async def list_trades(
            ...
            with self._log_execution_time("list_trades", current_user.id):
                user_trades = [
        ...
        @self.app.get("/trades/{trade_id}", response_model=TradeResponse)
        async def get_trade(
            trade_id: str = Path(..., title="The ID of the trade to get"),
            current_user: User = Depends(self._get_current_user)
        ):
        ...
        async def delete_trade(
            trade_id: str = Path(..., title="The ID of the trade to delete"),
            current_user: User = Depends(self._get_current_user)
        ):
            if trade_id not in self.trades:
                raise HTTPException(status_code=404, detail="Trade not found")
        ...
        # Analytics Routes
        @self.app.get("/analytics/history", response_model=HistoricalAnalytics)
        async def get_historical_analytics(
            ...
            with self._log_execution_time("get_historical_analytics", current_user.id):
                ...
                # Calculate analytics
                successful_trades = len([t for t in user_trades if t.status == TradeStatus.COMPLETED])
                total_trades = len(user_trades)
                
                analytics = HistoricalAnalytics(
                    total_trades=total_trades,
                    success_rate=successful_trades / total_trades * 100,
                    average_return=sum(t.performance_metrics.get('return_percentage', 0) for t in user_trades) / total_trades,
                    total_allocation=sum(t.task.allocation for t in user_trades),
    ...
    def run(self, host: str = "0.0.0.0", port: int = 8000, *args, **kwargs):
        """Run the FastAPI application"""
        import uvicorn
        logger.info(f"Starting Enhanced AutoHedge API on {host}:{port}")
